...
fn remote_callbacks<'a>(custom_key: Option<PathBuf>) -> git2::RemoteCallbacks<'a> {
    ...
    cb.credentials(move |url, username_from_url, allowed_types| {
        if allowed_types.contains(CredentialType::SSH_KEY) {
            if let Some(user) = username_from_url {
                ...
                // An explicit user-chosen key (generated by uGit, or picked
                // via "Use existing key…") takes priority over the guessed
                // default filenames below.
                let mut candidates: Vec<PathBuf> = Vec::new();
                if let Some(custom) = &custom_key {
                    candidates.push(custom.clone());
                }
                let ssh_dir = crate::ssh::ssh_dir();
                for key in crate::ssh::DEFAULT_KEY_NAMES {
                    candidates.push(ssh_dir.join(key));
                }
...
pub fn clone_repository(
    ...
    fetch_opts.remote_callbacks(remote_callbacks(custom_key));
...
fn fetch_via_https_fallback(
    ...
    opts.remote_callbacks(remote_callbacks(custom_key));
...
pub fn fetch(
    ...
    match r.fetch(&[] as &[&str], Some(&mut opts), None) {
        Err(e) => fetch_via_https_fallback(&repo, r.url(), &[], &e, custom_key),
        Ok(()) => Ok(()),
    }
}

#[tauri::command]
pub fn pull(app: tauri::AppHandle, repo_path: String) -> Result<String, String> {
    let repo = open_repo(&repo_path)?;
    ...
    match r.fetch(&[refspec.as_str()], Some(&mut opts), None) {
        Err(e) => fetch_via_https_fallback(&repo, r.url(), &[refspec.as_str()], &e, custom_key)?,
        Ok(()) => {}
    }
